...
func TestGetRandomBoss(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if boss == nil {
		t.Fatal("expected boss to be non-nil")
	}

	if boss.Name == "" {
		t.Error("expected boss to have a name")
	}

	t.Logf("Random boss selected: %s", boss.Name)
}

func TestGetRandomSkill(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	t.Logf("Random skill selected: %s", skill.Name)
}
